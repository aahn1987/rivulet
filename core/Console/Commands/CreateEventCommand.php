<?php
namespace Rivulet\Console\Commands;

use Rivulet\Console\Command;

class CreateEventCommand extends Command
{
    protected string $name        = 'create:event';
    protected string $description = 'Create a new event';

    public function execute(array $args): void
    {
        $name = $this->getArg($args, 0);

        if (empty($name)) {
            $this->error('Event name is required');
            return;
        }

        $parsed        = $this->parseName($name);
        $className     = $this->studlyCase($parsed['class']);
        $namespace     = $parsed['namespace'] ? $parsed['namespace'] : 'Events';
        $fullNamespace = "App\\{$namespace}";

        $stub    = $this->getStub('event');
        $content = $this->replaceInStub($stub, [
            'namespace' => $fullNamespace,
            'class'     => $className,
        ]);

        $path = appPath($parsed['path'] . '/' . $className . '.php');

        if ($this->createFile($path, $content)) {
            $this->success("Event {$className} created successfully at {$path}");
        } else {
            $this->error("Failed to create event {$className}");
        }
    }
}
